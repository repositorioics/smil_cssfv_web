...
import TipoMuestras from '../../../components/catalogo/tipo-muestras/TipoMuestras';
import DataServices from '../../../service/Api';
import ToastContainer from '../../../components/toast/Toast';
//import { useAuth } from '../../../context/Auth';

const TipoMuestrasContainer = props => {
    const [title, setTitle] = useState('Agregar tipo de muestra');
    const [titleForm] = useState('Lista tipos de muestras');
    ...
    const handleShow = () => {
        setShow(true);
        setIsNew(true);
        setTitle('Agregar tipo de muestra');
        setErrorMessageName('');
        setName('');
        setDescription('');
        setIsChecked(false);
    ...
    const tableRowEvents = {
        onClick: (e, row, rowIndex) => {
            setTitle('Editar tipo de muestra');
            setName(row.nombre);
    ...
    const saveTipoMuestra = async () => {
        try {
            const catTipoMuestra = {
                nombre: name,
                descripcion: description,
                activo: isChecked
            }

            const response = await DataServices.postTipoMuestras(catTipoMuestra);
            if (response.status === 200) {
                setType("success");
    ...
    const editTipoMuestra = async () => {
        try {
            ...
            const response = await DataServices.putTipoMuestras(catTipoMuestra);
            if (response.status === 200) {
                setType("success");
...
export default TipoMuestrasContainer;