...
import MotivoAnulacion from '../../../components/catalogo/motivo-anulacion/MotivoAnulacion';
import DataServices from '../../../service/Api';
import ToastContainer from '../../../components/toast/Toast';
//import { useAuth } from '../../../context/Auth';

const MotivoAnulacionContainer = props => {
    const [title, setTitle] = useState('Agregar motivo de anulación');
    const [titleForm] = useState('Lista motivos de anulación');
    ...
    const handleShow = () => {
        setShow(true);
        setIsNew(true);
        setTitle('Agregar motivo de anulación');
        setErrorMessageName('');
        setName('');
        setDescription('');
        setIsChecked(false);
    ...
    const getAll = async () => {
        try {
            const response = await DataServices.getAllMotivos();
    ...
    const tableRowEvents = {
        onClick: (e, row, rowIndex) => {
            setTitle('Editar motivo de anulación');
            setName(row.nombre);
    ...
    const saveMotivo = async () => {
        try {
            const catMotivoAnulacion = {
                nombre: name,
                descripcion: description,
                activo: isChecked
            }

            const response = await DataServices.postMotivo(catMotivoAnulacion);
            if (response.status === 200) {
                setType("success");
    ...
    const editMotivo = async () => {
        try {
            ...
            const response = await DataServices.putMotivo(catMotivoAnulacion);
            if (response.status === 200) {
                setType("success");
...
export default MotivoAnulacionContainer;