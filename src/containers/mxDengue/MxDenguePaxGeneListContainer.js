...
import MxDenguePaxGeneList from '../../components/mxDengue/MxDenguePaxGeneList';
...
const MxDenguePaxGeneContainer = props => {
    let history = useHistory();
    const [titleForm] = useState('Muestras Dengue PaxGene');
    ...
    const getMxDengueList = async () => {
        ...
        try {
            const response = await DataServices.muestrasDenguePaxGene();
...
/*         if (selectedMotivo > 0 && ckOtroMotivo) {
    ...
    const editMxDengue = (row) => {
        if (row.estado === "Activa") {
            history.push(`/muestras/editar-muestra-dengue/${row.id}/${'paxgene'}`);
        }
    }
    ...
    const searchData = () => {
        if (validateSearchData()) {
            if (validateDates()) {
                getMxDengueListByFilter(code === '' ? 0 : code, startDate, endDate, 'paxgene');
            }
...
export default MxDenguePaxGeneContainer;