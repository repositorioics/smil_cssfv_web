...
import MetabolomicaList from '../../components/mxDengue/MetabolomicaList';
...
const MxDengueMetabolomicaContainer = props => {
    let history = useHistory();
    const [titleForm] = useState('Muestras Dengue Metabolomicas');
    ...
    const getMxDengueList = async () => {
        ...
        try {
            const response = await DataServices.muestrasDengueMetabolomicas();
...
/*         if (selectedMotivo > 0 && ckOtroMotivo) {
    ...
    const editMxDengue = (row) => {
        if (row.estado === "Activa") {
            history.push(`/muestras/editar-muestra-dengue/${row.id}/${'metabolomica'}`);
        }
    }
    ...
    const searchData = () => {
        if (validateSearchData()) {
            if (validateDates()) {
                getMxDengueListByFilter(code === '' ? 0 : code, startDate, endDate, 'metabolomica');
            }
...
export default MxDengueMetabolomicaContainer;