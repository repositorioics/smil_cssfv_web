...
import MxDenguePbmcList from '../../components/mxDengue/MxDenguePbmcList';
...
const MxDenguePbmcContainer = props => {
    let history = useHistory();
    const [titleForm] = useState('Muestras Dengue PBMC');
    ...
    const getMxDengueList = async () => {
        ...
        try {
            const response = await DataServices.muestrasDenguePbmc();
...
/*         if (selectedMotivo > 0 && ckOtroMotivo) {
    ...
    const searchData = () => {
        if (validateSearchData()) {
            if (validateDates()) {
                getMxDengueListByFilter(code === '' ? 0 : code, startDate, endDate, 'pbmc');
            }
...
export default MxDenguePbmcContainer;