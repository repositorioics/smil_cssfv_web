...
import MxTransmisionList from '../../components/mxTransmision/MxTransmisionList';
...
const MxTransmisionListContainer = props => {
    let history = useHistory();
    const [titleForm] = useState('Muestras de serología - Estudio transmisión');
    ...
    const getMxTransmisionList = async () => {
        ...
        try {
            const response = await DataServices.getMuestrasTransmision(Constants.ID_MUESTRA_TRANSMISION);
    ...
    const getMxTransmisionListByFilter = async (code, startDate, endDate) => {
        setExecuteLoading(true);
        try {
            const response = await DataServices.filtroMxTransmision(code, startDate, endDate, Constants.ID_MUESTRA_TRANSMISION);
            if (response.status === 200) {
                setExecuteLoading(false);
                const newData = [];
...
/*         if (selectedMotivo > 0 && ckOtroMotivo) {
    ...
    const editMxTransmision = (row) => {
        if (row.estado === "Activa") {
            history.push(`/muestras/editar-muestra-transmision/${row.id}`);
        }
    }
...
export default MxTransmisionListContainer;